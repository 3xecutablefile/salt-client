...
public final class OptionTweaks {
    ...
    public static void tick(MinecraftClient mc) {
        ...
        if (lowGraphicsMode) {
            ...
            Module fbMod = SaltClient.MODULES.byId("fpsboost").orElse(null);
            FpsBoostModule.Level lvl = (fbMod instanceof FpsBoostModule fbm)
                ? fbm.getLevel()
                : FpsBoostModule.Level.BALANCED;

            // Every level turns off the two cheapest tweaks.
            ao = false;
            entityShadows = false;
